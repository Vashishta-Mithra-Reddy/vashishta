...
export default function Dock() {
  ...
  return (
    <section className="hidden md:block">
      <nav className="fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-white border-2 px-8 py-5 backdrop-blur-xl rounded-xl space-x-2">
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item}
              href={itemHash}
              className={`nav-link relative group text-sm font-semibold tracking-wide px-4 py-2 rounded-lg font-poppins transition-colors duration-300 ${
